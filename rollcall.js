...
var mep_rollcall = streamCSV(
  config.folder + "mep_rollcall.csv",
  "mepid,mep,result,group,identifier"
);
var item_rollcall = streamCSV(
  config.folder + "item_rollcall.csv",
...
var csvParser = csv
  .fromPath("./data/rollcall.csv", { headers: true })
  .on("data", function (d) {
...
function transformFile(d) {
  const file =
    "./data/" + d.code + "/" + d.baseurl.split("/").pop() + ".xml.zip";
  return new Promise((resolve, reject) => {
    ...
    unzip.setEncoding("utf8").on("error", (err) => {
    ...
    var vote = {
      push: function (k, v) {
        if (this[k]) {
          console.log(this);
          console.error("value for " + k + " already set to " + this[k]);
        }
        this[k] = v;
      },
      pop: function (k) {
    ...
    xml.on("updateElement: RollCallVote.Description.Text", (i) => {
      ...
      if (i.$text.charAt(0) == "-") i.$text = i.$text.substring(1);
    ...
    xml.on("updateElement: PoliticalGroup.Member.Name", (mep) => {
      ...
      ["result", "group", "identifier", "date", "desc"].map((i) => {
        //group not mandatory, more QA
        t[i] = vote[i];
      });
    ...
    xml.on("updateElement: Member.Name", (mep) => {
      ...
      if (!mep.$ || !mep.$.MepId) {
        console.log(vote);
        console.log(
          "Error in MEP record (no id) for " + mep.$text + " in " + d.reference
        );
      }
      ...
      if (mep.$.MepId == "DUPLICATE") {
        console.error(
          d.reference + ":duplicate " + mep.$text + " in " + vote.identifier
        ); // + " "+vote.url);
        return;
      }
      var t = { mepid: mep.$.MepId, mep: mep.$text };
