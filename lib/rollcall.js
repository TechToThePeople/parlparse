...
const processRollCall = async (plenary, options) => {
  ...
  const forcePosition = options.position;
  const folder = options.folder || "./data/RCV/";
  const formatPosition = async (vote, result, group, d) => {
    //const id = +d.$.MepId; we don't match on the vote id but on the ep_id
    const id = +d.$.PersId;
    if (!isMep(id)) {
  ...
  const vote = async (d) => {
    ...
    if (d["RollCallVote.Description.Text"][0]._) {
      ...
      if (
        ...
        let r = name.match(/(.*) - ([A|B|C]\10-\d{4}\/20\d\d) - (.*)/);
        if (r) {
          ref = r[2];
          name = r[3];
        ...
        if (name.includes("Ordre du jour") || name.includes(" agenda")) {
          ref = "agenda week " + getWeek(new Date(d.$.Date));
        }
        let split = name.split(" – ");
        if (split.length === 1) split = name.split(" - ");
        if (split.length > 1) {
          ref = split.shift().trim();
          name = split.join("-").trim();
        }
        //console.log("nothing at the bottom");
      }
    }
    ...
    try {
      ...
      if (parseInt(dd[0].count) === 1) {
        ...
        const positions = await db("positions")
          .count("*")
          .where("rollcall", v.id);
        positions[0].count = parseInt(
          positions[0].count || positions[0]["count(*)"]
        );
    ...
    unzip
      ...
      .on("end", () => {
        parser.parseString(xml, async (err, d) => {
          const rcvresult = d["PV.RollCallVoteResults"];
